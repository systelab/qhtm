...
void CHTMLList::AddDisplayElements( class CHTMLSectionCreator *psc )
{
	...
	*/
	...
	const int nBulletWidth = MulDiv( m_bCompact ? g_defaults.m_nIndentSize / 2 : g_defaults.m_nIndentSize, GetDeviceCaps( psc->GetDC().GetSafeHdc(), LOGPIXELSX), 1000); 
	const int nBulletSpace = MulDiv( m_bCompact ? g_defaults.m_nIndentSpaceSize / 2 : g_defaults.m_nIndentSpaceSize, GetDeviceCaps( psc->GetDC().GetSafeHdc(), LOGPIXELSX), 1000); 

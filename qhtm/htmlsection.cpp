...
void CHTMLSection::AddBreakSection(int i, int row /* = -1 */)
{
...
int CHTMLSection::Paginate(const WinHelper::CRect& rcPage, int *pIntArray /* = 0 */)
{
	/*	Strategy:
	...
	*/
	...
	else
	{
		...
		for (nBreakIndex = 0; nBreakIndex < nPageBreaks; ++nBreakIndex)
		{
			...
			const int nRow = m_arrBreakRow[nBreakIndex];
			if (nRow >= 0)
			{
				currentRow = nRow;
			}
...
{
	_ASSERT(nPage < m_arrPageRects.GetSize());
...
void CHTMLSection::SubmitForm( CHTMLForm *pForm )
{
...
bool CHTMLSection::SubmitForm( LPCTSTR pcszFormName )
{
	if( m_pDocument )
	{
		CHTMLForm *pFoundForm = NULL;

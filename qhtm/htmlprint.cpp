...
BOOL WINAPI QHTM_PrintLayout( QHTMCONTEXT ctx, HDC hDC, LPCRECT pRect, LPINT nPages, int *pIntArray /* = 0 */)
{
#ifdef QHTM_ALLOW_PRINT
	CQHTMPrintContext** ppctx = g_mapPrintContext.Lookup( ctx );
	if (ppctx)
	{
		...
		ctx.m_nPages = *nPages = ctx.m_htmlSection.Paginate( ctx.m_rcLayout, pIntArray);
		return TRUE;
	}
	else
#endif	//	QHTM_ALLOW_PRINT

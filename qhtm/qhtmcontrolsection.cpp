...
//	_ASSERT( m_mapTimerEvents.GetSize() == 0 );
//#endif	//	_DEBUG
}
...
void CQHTMControlSection::OnVScroll( int nScrollCode )
{
	...
	const int nLineScrollAmount = m_htmlSection.GetSize().cy / 10;
...
void CQHTMControlSection::OnHScroll( int nScrollCode )
{
	...
	const int nLineScrollAmount = m_htmlSection.GetSize().cx / 10;
...
void CQHTMControlSection::UnregisterTimerEvent( const int nTimerEventID )
{
	...
	_ASSERT( nTimerEventID != knNullTimerId );
...
bool CQHTMControlSection::SubmitForm( LPCTSTR pcszFormName )
{
	return m_htmlSection.SubmitForm( pcszFormName );
}

...
void CHTMLTableLayout::GetDesiredWidths()
{
	...
	int tabWidth = m_nTableWidth - 2 * m_pTab->m_nBorder;
	for (int nRow = 0; nRow < m_nRows; nRow++)
	{
		CHTMLTable::CHTMLTableRow *pRow = m_pTab->m_arrRows[ nRow ];
		int widthTot = 0;
		for( UINT nCol = 0; nCol < pRow->m_arrCells.GetSize(); nCol++ )
		{
			...
			if (nDesiredWidth != 0)
			{
				if( nDesiredWidth < 0 )
				{
					...
					{
						nWidth = min( (int)( (double)tabWidth / 100.0 * abs(nDesiredWidth) + 0.4999 ), tabWidth );
					}
				}
				else
				{
				...
				if (nCol == pRow->m_arrCells.GetSize() - 1)
				{
					nWidth += tabWidth - widthTot;
				}
				nWidth -= (m_pTab->m_nBorder + m_pTab->m_nCellPadding) * 2 + m_pTab->m_nCellSpacing;
...
int CHTMLTableLayout::CalculateTableSize()
{
	if( m_pTab->m_nWidth < 0 )
	{
		...
		{
			m_nTableWidth = min( (int)( (double)m_nMaxWidth / 100.0 * abs( m_pTab->m_nWidth ) + 0.4999 ), m_nMaxWidth );
		}
	}
...
void CHTMLTableLayout::LayoutAllSpace(int nSpaceAvailable, int nSpaceNeeded)
{
	...
	if (nSpaceAvailable > 0)
	{
		...
		{
			//	This might seem wrong, but remember that the case where all cells

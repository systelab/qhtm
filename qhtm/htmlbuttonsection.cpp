...
void CHTMLButtonSection::OnDraw( GS::CDrawContext &dc )
{
	if( dc.IsPrinting() )
	{
		...
		case CHTMLFormInput::knRadio:
			DrawFrameControl( dc.GetSafeHdc(), this, DFC_BUTTON, DFCS_BUTTONRADIO );
			break;

		case CHTMLFormInput::knCheckbox:
			DrawFrameControl( dc.GetSafeHdc(), this, DFC_BUTTON, DFCS_BUTTONCHECK );
			break;
		}		
	}
	else
	...
	case WM_COMMAND:
		...
		case BN_CLICKED:
			...
			case CHTMLFormInput::knSubmit:
				if( m_pFormObject->m_pForm )
				{
					m_psectHTMLParent->SubmitForm( m_pFormObject->m_pForm );
				}
				break;

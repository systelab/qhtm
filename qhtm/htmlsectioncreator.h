...
public:
	...
	WinHelper::CSize GetSize() const
	{
		return WinHelper::CSize( m_nWidth, m_nYPos );
	}
	...
	CHTMLSectionLink* GetCurrentLink() const
	{
		return m_pCurrentLink;
	}

	//
	//	Gain access to the underlying HTML display
	CHTMLSection *GetHTMLSection() const
	{
		return m_psect;
	}
	...
	//
	//	Y-Position control
	void AddYPos( int nToAdd )
	{
		m_nYPos += nToAdd;
	}
	void SetCurrentYPos( int nYPos )
	{
		m_nYPos = nYPos;
	}
	...
	//
	//	X-Position control
	void SetNextYPos( int nYPos )
	{
		m_nNextYPos = nYPos;
	}
	int GetNextYPos() const
	{
		return m_nNextYPos;
	}
	...
	{
		return m_nRightMargin - m_nLeftMargin;
	}
	...
	int GetDefaultRightMargin() const
	{
		return m_nDefaultRightMargin;
	}
	int GetDefaultLeftMargin() const
	{
		return m_nDefaultLeftMargin;
	}

	void AddBreak(int row = -1)
	{
		m_psect->AddBreakSection( GetCurrentShapeID(), row);
	}

	void SetCurrentLink( CHTMLSectionLink* pCurrentLink )
	{

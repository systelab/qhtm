...
CQHTMImageABC *AquireImage( HINSTANCE hInstance, LPCTSTR pcszFilePath, LPCTSTR pcszFilename, bool bIsTransparent, COLORREF crForceTransparent )
{
	...
	if( !_tcsnicmp( pcszFilename, _T("RES:"), 4 ) )
	{
		...
		HMODULE hOther = NULL;
		//LPTSTR pszNumber = _tcsrchr( pcszFilename, ':' );  //line commented by costin
		LPCTSTR pszNumber = _tcsrchr( pcszFilename, ':' );   //line added by costin
		if( pszNumber )
		{			
			// josep: The following changes by costin are due to the fact that pcszFilename is a constant string ( LPCTSTR ),  
			// and thus we cannot modify it directly ( nor using a temporary pointer that points to some part of the string )
			StringClass tmp;	//line added by costin
			tmp.Set( pcszFilename, pszNumber-pcszFilename ); //line added by costin
			// *pszNumber = '\000'; //line commented by costin
			//hOther = LoadLibraryEx( pcszFilename, NULL, LOAD_LIBRARY_AS_DATAFILE ); //line commented by costin
			hOther = LoadLibraryEx( tmp.GetData(), NULL, LOAD_LIBRARY_AS_DATAFILE ); //line added by costin

...
bool CArray<T>::SetSize( PI_uint32 uSize )
{
	...
	_ASSERT( m_uItemCount <= m_uItemsAllocated );
...
void CArray<T>::InsertAt( PI_uint32 iPos, const T *p, PI_uint32 uCount )
{
	if( p )
	{
		...
		//_ASSERT( !IsBadReadPtr( p, sizeof( T ) * uCount ) );
...
{
	_ASSERT( iPos + uItems <= m_uItemCount );
	_ASSERT( uItems > 0 );

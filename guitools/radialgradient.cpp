...
void GS::RadialGradient( GS::CDIB &dib, const WinHelper::CRect &rc, GS::COLORREFA crFrom, GS::COLORREFA crTo, int nGradientX, int nGradientY, int nGradientRadius )
{
	...
	register const int nBottom = min( rc.bottom, dib.GetHeight() );
	const int nLeft = max( rc.left, 0 );
	register const int nRight = min( rc.right, dib.GetWidth() );
  ...
  for ( int nY = nTop; nY < nBottom; nY++)
	{
		...
		double dPreY = pow( static_cast<double>(nY - nTop - nGradientY), 2);
		for ( int nX = nLeft; nX < nRight; nX++)
		{
			nGradient = ((nGradientRadius - (int)sqrt( pow(static_cast<double>(nX - nLeft - nGradientX), 2) + dPreY )) * 100) / nGradientRadius;
